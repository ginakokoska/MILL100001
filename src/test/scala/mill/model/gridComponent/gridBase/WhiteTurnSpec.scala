...
case class WhiteTurnSpec() extends AnyWordSpec with Matchers {
  "WhiteTurn" when {
    ...
    "called with setStoneState() should return" should {
      val state = WhiteTurn().setStoneState("OS: 00", tmpGrid, controller.player1)
      val state1 = WhiteTurn().setStoneState("OS: 00", tmpGrid, controller.player1)
      WhiteTurn().setStoneState("MS: 00", tmpGrid, controller.player1)
      WhiteTurn().setStoneState("MS: 01", tmpGrid, controller.player1)
      val stateMill = WhiteTurn().setStoneState("MS: 02", tmpGrid, controller.player1)
    ...
    "moveStoneState() should return" should {
      ...
      WhiteTurn().setStoneState("OS: 00", tmpGrid, controller.player1)
      WhiteTurn().setStoneState("MS: 01", tmpGrid, controller.player1)
      WhiteTurn().setStoneState("OS: 12", tmpGrid, controller.player1)
      val state = WhiteTurn().moveStoneState("move MS: 01 to OS: 01", tmpGrid, controller.player1)
      val state1 = WhiteTurn().moveStoneState("move OS: 00 to OS: 02", tmpGrid, controller.player1)
      val stateMill = WhiteTurn().moveStoneState("move OS: 12 to OS: 02", tmpGrid, controller.player1)
    ...
    "jumpStone(), should return" should {
      tmpGrid.createFullGrid()
      WhiteTurn().setStoneState("OS: 00", tmpGrid, controller.player1)
      WhiteTurn().setStoneState("OS: 01", tmpGrid, controller.player1)
      WhiteTurn().setStoneState("IS: 00", tmpGrid, controller.player1)
      val state = WhiteTurn().jumpStoneState("jump OS: 00 to OS: 02", tmpGrid, controller.player1)
      val state1 = WhiteTurn().jumpStoneState("jump MS: 00 to MS: 02", tmpGrid, controller.player1)
      val stateMill = WhiteTurn().jumpStoneState("jump IS: 00 to OS: 00", tmpGrid, controller.player1)
