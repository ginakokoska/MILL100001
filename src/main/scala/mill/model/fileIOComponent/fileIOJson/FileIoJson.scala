...
class FileIO extends FileIoInterface {
  ...
  override def load(controller: ControllerInterface): List[Array[Array[Node]]] = {
    ...
    val p1List: List[String] = List(p1.get("name").toString(), p1.get("color").toString())
    val p2List: List[String] = List(p2.get("name").toString(), p2.get("color").toString())
              ...
              Json.obj(
                ...
                if (controller.grid.gridList(sq)(row)(col).isSet) {
                  if (controller.grid.gridList(sq)(row)(col).isColor.contains(Stone.white)) {
                    val strPos = row.toString + col.toString + "white"
                    "color" -> 1
                  } else {
                    val strPos = row.toString + col.toString + "black"
                    "color" -> 2
                  }
                } else {
                  "color" -> 0
